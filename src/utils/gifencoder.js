...
class GifEncoder {
  constructor(opts) {
    ...
    this.writer = new Worker("/public/js/webwork/gif/writer.js");
    this.writer.postMessage(opts);
    const onMessage = msg => this._onWriterMessage(msg);
    this.writer.addEventListener("message", onMessage);
    this.disposeWriter = () =>
      this.writer.removeEventListener("message", onMessage);
    const numberOfWorkers = navigator.hardwareConcurrency
      ? Math.floor(navigator.hardwareConcurrency * 0.8)
      : 4;
    for (let i = 0; i < numberOfWorkers; i++) {
      const worker = new Worker("/public/js/webwork/gif/quantizer.js");
